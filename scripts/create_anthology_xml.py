	...
	def from_line(acronym, line):
		...
		if len(parts) == 11:
			...
			if note=="none" or note.strip()=="":
				note=""
			return Entry(bibtype, authors, title, booktitle, month, year, publisher, pages, url, note, paperID, acronym)
		else:
			raise(ValueError("WARNING:\tLine should contain 10 columns, but contains " + len(parts)+"\t"+line))
			
	def __init__(self, bibtype, authors, title, booktitle, month, year, publisher, pages, url, note, paperID, acronym):
